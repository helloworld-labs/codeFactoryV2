...
<form data="editForm" class="form-horizontal" id="saveObjForm">
 ...
 <div class="form-group">
     ...
     <input type="text"  name="${table.fieldNames[i_index]}"  placeholder="请输入${table.fieldNames[i_index]}" value="${r"${"}${table.entityName?uncap_first}.${table.fieldNames[i_index]}}" class="form-control"/>
     </#if>
     <#if table.filedTypes[i_index] == "Date">
     <input type="text" class="Wdate" onClick="WdatePicker()"  name="${table.fieldNames[i_index]}"  placeholder="日期" value="<fmt:formatDate value="${r"${"}${table.entityName?uncap_first}.${table.fieldNames[i_index]}}" pattern="yyyy-MM-dd HH:mm:ss"/>" class="form-control"/>
     </#if>
     </div>
 </div>
     </#if>
